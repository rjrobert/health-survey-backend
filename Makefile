...
BIN := health-survey-backend

# Where to push the docker image.
REGISTRY ?= docker.pkg.github.com/rjrobert/health-survey-backend
...
test: $(BUILD_DIRS)
	@docker run                                                 			\
	    ...
	    -v $$(pwd)/.go/cache:/.cache                            			\
	    -v $$(pwd)/config:/config                               			\
	    -v $$(pwd)/cmd/health-survey-backend/test_data:/test_data           \
	    --env HTTP_PROXY=$(HTTP_PROXY)                          			\
	    --env HTTPS_PROXY=$(HTTPS_PROXY)                        			\
	    --env HEALTH_SURVEY_API_KEY=DUMMY                           		\
	    --env HEALTH_SURVEY_DSN=DUMMY                               		\
		--env HEALTH_SURVEY_JWT_AUDIENCE=DUMMY								\
		--env HEALTH_SURVEY_JWT_ISSUER=DUMMY								\
		--env HEALTH_SURVEY_JWT_JWKS=DUMMY									\
	    $(TEST_IMAGE)                                           			\
	    /bin/sh -c "                                            			\
	        ARCH=$(ARCH)                                        			\
	        OS=$(OS)                                            			\
